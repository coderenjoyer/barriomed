...
CREATE TABLE IF NOT EXISTS patient_medical_info (
    ...
    profile_picture_url TEXT,                       -- public URL from user-profile bucket
...
-- Drop existing table policies before recreating
DROP POLICY IF EXISTS "Authenticated users can read medical info" ON patient_medical_info;
DROP POLICY IF EXISTS "Users can insert own medical info" ON patient_medical_info;
DROP POLICY IF EXISTS "Users can update own medical info" ON patient_medical_info;
...
-- Create the user-profile storage bucket (public)
